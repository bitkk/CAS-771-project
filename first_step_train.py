...
noise_file='%s/cifar10_noisy_labels_task3.json' % (data_path)
noise_label = json.load(open(noise_file, "r"))
noise_labels = np.array(noise_label)
############################
...
def train_val_split(train_val, train_n):
    ...
    train = train_val[train_key]
    noise = noise_labels[train_key]
    train[:, 1] = noise
    return train, train_val[val_key]
